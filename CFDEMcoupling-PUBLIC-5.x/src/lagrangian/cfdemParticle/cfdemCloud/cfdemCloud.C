...
// * * * * * * * * * * * * * * * protected Member Functions (not private functions) * * * * * * * * * * * * * //
...
tmp<fvVectorMatrix> cfdemCloud::divVoidfractionTau(volVectorField& U,volScalarField& voidfraction) const
{
    ...
    (
      ...
      - fvc::div(voidfractionNuEff(voidfraction)*dev2(fvc::grad(U)().T()))   //kinematic viscosity
    );
}
...
tmp<volScalarField> cfdemCloud::voidfractionNuEff(volScalarField& voidfraction) const
{
    if (modelType_=="B" || modelType_=="Bfull")
    {
        ...
        (
            #ifdef compre
                new volScalarField("viscousTerm", (  turbulence_.muEff() //t()  
                                                  // + turbulence_.mu() 
                                                  // + turbulenceMultiphase_
                                                  )
                                  )
    ...
    else
    {
        ...
        (
            #ifdef compre
                new volScalarField("viscousTerm", voidfraction*(  turbulence_.muEff()   //.mut() 
                                                                //+ turbulence_.mu()
                                                                //+ turbulenceMultiphase_
                                                               )
                                  )
...
/*
tmp<volScalarField> cfdemCloud::voidfractionMuEff(volScalarField& voidfraction) const
{
    if (modelType_=="B" || modelType_=="Bfull")
    {
        return tmp<volScalarField>
