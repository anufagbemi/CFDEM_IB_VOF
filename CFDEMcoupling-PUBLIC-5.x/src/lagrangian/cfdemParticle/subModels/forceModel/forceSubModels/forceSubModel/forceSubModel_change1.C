...
/*
    ...
    (
        ...
        sm.mesh(),
        dimensionedVector("IBDragPerV_VoF", dimensionSet(1, -2, -2, 0, 0), vector::zero)
    ),
*/
...
/*
const volVectorField& forceSubModel::IBDragPerV_VoF(const volectorField& U, const volScalarField& p) const // NEW !
{
    #ifdef compre
        IBDragPerV_VoF = muField()*fvc::laplacian(U) - fvc::grad(p)/rhoField();
    #else
	IBDragPerV_VoF = rhoField()*(nuField()*fvc::laplacian(U) - fvc::grad(p))/rhoField();
    #endif
    return IBDragPerV_VoF;
